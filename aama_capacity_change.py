...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 12 00:55:27 2023
...
## Random First Improvement Algorithm with capacity change
...
# Step 2:
# increase facility capacity by 10%
capacity_new = facility.loc[facility.index[:], facility.columns[3]]*1.1
...
while iteration <= max_ite and non_empty_solution:
    ...
    # try to allocate customer to a different facility
        ...
        # check if facility j has enough spare capacity
                if demand[random_customer] <= capacity_new[j]:
                ...
                # if yes, reallocate customer to facility j
                        ...
                        capacity_new[random_facility] += demand[random_customer]
                        capacity_new[j] -= demand[random_customer]
     
                # start over with a new randomly selected customer
                        break
                    iteration += 1
    # check if minimum allocation cost is updated
        elif current_allocation_cost < min_allocation_cost[k]:
            min_allocation_cost[k] = current_allocation_cost

total_cost2 = 0
# Step 4: Print total cost
for i in range(len(demand)):
    for j in range(len(capacity_new)):
        total_cost2 += np.sum(allocation[i][j]*cost[i][j]*demand[i])
print("Total cost_FI_increase_capacity: ", total_cost2)
